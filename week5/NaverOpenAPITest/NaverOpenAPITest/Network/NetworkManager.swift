...
class NetworkManager {
    ...
    func fetchDataList(path: String, query: String, fetchDataInBackground: @escaping (_ parsedData: Response) -> Void) {
        let urlComponents = fillUrlComponents(path: path, query: query)
        guard let url = urlComponents.url else {
            return
        }
        let urlRequest = fillUrlReuqest(url: url, httpMethod: "GET")
        let task = URLSession.shared.dataTask(with: urlRequest) { data, response, error in
            ...
            guard let data = data, let parsedData = try? JSONDecoder().decode(Response.self, from: data) else {
                print("data error!")
                return
            }
